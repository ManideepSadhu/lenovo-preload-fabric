...
module.exports.getClientForOrg = async function (org) {
	if (clients[org] == undefined) {
		...
		let cryptoSuite = new pkcs11CryptoSuite(256, 'SHA2');
...
module.exports.getEventHubsForOrg = function (channel, userOrg, peer) {
	let peerName = peer.getName();
	let eventHub = eventHubs[userOrg + '-' + peerName];

	if (eventHub == null || eventHub === undefined) {
		eventHubs[userOrg + '-' + peer.getName()] = channel.newChannelEventHub(peer);
		eventHub = eventHubs[userOrg + '-' + peerName]
	}

	if (!eventHub.isconnected()) {
		logger.debug("Event hub not connected yet: " + eventHub.getPeerAddr());
		eventHub.connect(true);
	} else {
		logger.debug("Event hub already connected: " + eventHub.getPeerAddr());
	}
	return eventHub;
}
